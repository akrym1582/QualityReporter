...
{
    private static readonly string[] ScoreNames = ["activity", "maintainability", "testability", "architecture", "knowledge", "overallQuality", "hotspotPriority"];
    public static QualitySnapshot Build(IEnumerable<string> reports, string sourceCommit, DateTimeOffset generatedAt, string version, string modelVersion, string? policyPath)
    {
        ...
        var repository = new SnapshotEntity { Scores = ScoreNames.ToDictionary(x => x, Median), Metrics = new()
        {
            ...
            ["untestedComplexCount"] = actualFiles.Count(x => x.Metrics.GetValueOrDefault("untestedComplexity") >= 15),
            ["duplicateHotspotCount"] = actualFiles.Count(x => x.Metrics.GetValueOrDefault("duplicatedPercentage") >= .2)
        }};
    ...
    private static SnapshotEntity Entity(JsonNode node, string? path, string? symbolId = null, string? name = null) => new()
    {
        ...
        Metrics = new() { ["complexity"] = Number(node["metrics"]?["complexity"]), ["coverage"] = Percent(node["metrics"]?["lineCoverage"]),

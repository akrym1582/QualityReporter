...
public class V13MetricTests
{
 ...
 [Fact] public void Baseline_comparison_preserves_v1_trend_fields(){var current=new Report{Files=[new FileResult{Path="A.cs",Risk=new(){Score=80,Level="critical"},Hotspot=new(){PriorityScore=80}}]};var baseline=new Report{Files=[new FileResult{Path="A.cs",Risk=new(){Score=20,Level="low"},Hotspot=new(){PriorityScore=20}},new FileResult{Path="B.cs",Risk=new(){Score=90,Level="critical"},Hotspot=new(){PriorityScore=90}}]};var trend=BaselineComparer.Compare(current,baseline);var properties=trend.GetType().GetProperties().Select(x=>x.Name).ToHashSet();foreach(var name in new[]{"newHotspots","resolvedHotspots","riskIncreased","riskDecreased","priorityIncreased","priorityDecreased"})Assert.Contains(name,properties);Assert.Equal(["B.cs"],(IEnumerable<string>)trend.GetType().GetProperty("resolvedCritical")!.GetValue(trend)!);}
 [Fact] public void Empty_jscpd_report_records_zero_duplication(){var groups=new QualityReporter.CSharp.Duplication.JscpdReportParser().Parse("""{"duplicates":[]}""");var file=new FileResult{Path="src/A.cs",Metrics=new(){Loc=20},Symbols=[new SymbolResult{SymbolId=new string('a',64),SymbolKey="A.M",Name="M",Kind="method",Location=new(1,10),Metrics=new(){Loc=10}}]};QualityReporter.CSharp.Duplication.DuplicateSymbolMapper.Map(groups,[file]);Assert.Equal(0,file.Metrics.Duplication!.DuplicatedLines);Assert.Equal(0,file.Metrics.Duplication.DuplicatedPercentage);Assert.Equal(0,file.Symbols[0].Metrics.Duplication!.DuplicatedLines);}

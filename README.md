...
利用できないメトリクスは除外し、残りの重みを再正規化します。レベルは Critical（80 以上）、High（60 以上）、Medium（40 以上）、Low（40 未満）です。ホットスポットの順位はリスクだけでなく優先度を使用します。ベースラインは Critical／hotspot の状態、リスク、優先度の変化を示します。
...
`symbols` 配列は、現在の HEAD にあるメソッド、コンストラクター、演算子、設定されたアクセサー／ローカル関数、TypeScript の関数とメソッド、変数に代入された関数、React コンポーネント、カスタムフックを記述します。抽出対象と複雑度のしきい値は `methodAnalysis` で設定します。`functions` 配列には、レポート表示向けの関数メトリクスの要約を出力します。

シンボル履歴、カバレッジ／問題の割り当て、coupling、スコア、推奨事項、ベースラインの傾向は契約に含まれていますが、履歴マッピングが実装されるまでは利用できません。欠損値を品質の判定として解釈しないでください。デフォルトでは rename tracking が無効なため、メソッド名の変更で ID が分割されることがあります。

## 評価モデルと品質軸

QualityReporter は**開発活動**と**品質リスク**を分離します。Activity（変更頻度、churn、著者、新しさ）だけで問題を判定することはありません。ファイルは `ACTIVE`、`NEW_ACTIVE`、`COMPLEX`、`REWORK`、`UNTESTED`、`COUPLED`、`CRITICAL` に分類できます。推奨事項はメトリクスの根拠を含み、確定的な品質判定ではなく、レビュー、テスト、リファクタリング、設計レビューの候補を示します。

`scores` は次の品質軸を保持します。

* **Maintainability**: Complexity 30%、Rework 25%、Duplication 20%、Analyzer／Linter Issues 15%、Method Size 10%
* **Testability**: Untested Complexity 55%、Line Coverage 25%、Branch Coverage 20%
* **Architecture**: Change Coupling 70%、Analyzer／Linter Issues 30%
* **Overall Quality**: Maintainability 50%、Testability 30%、Architecture 20%
* **Hotspot Priority**: Overall Quality と成熟度調整済み Activity を組み合わせた改善優先度

各軸は利用可能な component だけで重みを再正規化し、計算不能な軸は `score: null` / `level: unknown` になります。Hotspot Priority は「現在の開発で改善効果が高い順」を示すもので、Activity が高いだけの健全なコードを問題とは判定しません。

## Untested Complexity

`untestedComplexity` は複雑度と未テスト割合を組み合わせます。カバレッジがない場合は `null`／`Not Available` とし、0% とみなしません。`coverageMode` は `line`（既定）、`branch`、`combined`（line 60% + branch 40%）から選択できます。複雑度自体も独立したレビューシグナルとして維持されます。

## 重複コード

重複検出アルゴリズムは QualityReporter に実装しません。CI で jscpd を実行し、その JSON を `--duplication` で渡します。QualityReporter は C#、TypeScript、TSX の fragment をファイルと symbol の行範囲へ割り当て、重複率と Activity を組み合わせてレビュー優先度を付けます。重複は共通化の確定判断ではなく、業務上同じ責務かを確認する候補です。
...
QualityReporter はバグ修正の意図を推測せず、GitHub API を呼び出さず、履歴を保存せず、メソッド単位の Git 履歴や rename を追跡しません。rename の解析は Git の numstat 出力に従い、バイナリの numstat エントリは churn に加算しません。複雑度や各リスクスコアはレビューと保守の優先順位付けに使うシグナルであり、意味論的な品質判定ではありません。
